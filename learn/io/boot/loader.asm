...
GdtPtr      dw GDTLen - 1               ;段界限
            dd BaseOfLoaderPhyAddr + LABEL_GDT  ;段基址

;GDT 选择子
SelectorFlatC   equ     LABEL_DESC_FLAT_C - LABEL_GDT
SelectorFlatRW  equ     LABEL_DESC_FLAT_RW - LABEL_GDT
SelectorVideo   equ     LABEL_DESC_VIDEO - LABEL_GDT + SA_RPL3 ; 显存并且在ring3

BaseOfStack     equ     0100h

LABEL_START:        ; Loader start here
    mov ax, cs  
    mov ds, ax
    mov ss, ax 
    mov es, ax 
...
; 找到文件后，我们需要把它加载到指定的位置
; 因为文件可能跨越几个扇区，所以我们根据扇区是否位最后一个扇区一个一个的寻找
; LOAD的方式是通过 INT 10H 这个中断
...
; InitKernel------------------------------------------------
; 将 kernel.bin 的内容整理后对齐放到新的位置
;-------------------------------------------------------
InitKernel: ;遍历每一个 program header，根据 program header的信息来确定把什么放进内存，放到什么位置， 以及放多少
    xor esi, esi
    mov cx, word [BaseOfKernelFilePhyAddr + 2ch] ; 条目的个数
    
    movzx ecx, cx 
    mov esi, [BaseOfKernelFilePhyAddr + 1ch] ;条目的偏移地址
    add esi, BaseOfKernelFilePhyAddr        ;基础地址
.Begin:
    mov eax,[esi + 0]
    cmp eax, 0
    jz  .NoAction       ; PT_NULL 下一个条目
    push dword [esi + 010h] ; size 
    mov eax, [esi + 04h]    ;offset
    add eax, BaseOfKernelFilePhyAddr 
    push eax 
    push dword [esi + 08h] ;vaddr
    call MemCpy
    add esp, 12
.NoAction:
    add esi, 020h
